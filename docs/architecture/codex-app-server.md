...
cross-agent boundary and the reason Codex, Claude, and Grok keep separate
native drivers are defined in [Agent Runtimes](agent-runtimes.md).
...
Codex, Claude, and Grok. Codex model, effort, speed, and permission choices are still
...
- The signer is configured without filesystem I/O and opened lazily only when
  ...
  its Claude and Grok drivers from starting. Tokens stay out of URLs, Task events, and
...
The last direct verification of this reconnect behavior used Codex CLI
`0.147.0`, below the current minimum supported baseline, `0.155.1`. The tests
are ignored by default because they require local authentication and make real
model requests; rerun them when changing the baseline or daemon/proxy
lifecycle.
...
Codex ships an updater that runs beside the daemon, installs each new release
from its `latest` channel, and restarts the daemon when the installed version
changes. That restart does not wait for running turns: work still running when
the daemon's shutdown grace period ends stops, in Caffold and in every other
client connected to the daemon. Caffold replaces the shared runtime only after
a person confirms it, so it turns that updater off.

Before each `codex app-server daemon start`, `restart`, and `update` it runs,
Caffold writes `updater.autoUpdateEnabled: false` into
`app-server-daemon/settings.json` under the Codex home, which is `CODEX_HOME`
when that is set and `~/.codex` otherwise. Codex stops a running updater once
it reads that value. Caffold writes only when the value is absent: a value that
is already `true` or `false` stays as it is, as does every other setting in the
file, so setting `true` there keeps Codex's updater. When the file cannot be
read as settings or cannot be written, Caffold leaves it as it is, logs the
problem, and still runs the daemon command.

Settings → Codex shows whether the updater is on and updates Codex only on
...
`unsupportedInstall`. Versions below `0.155.1` produce `updateRequired` before
...
tokens remain inside the Codex adapter. A null `runtimeStatus` is reported as
unavailable, and one thread's failed status read does not erase the other
thread snapshots.
...
`restartRequired`, except while a restart or an update, including its status
refresh, is in flight. Other readiness states do not establish a supported
restart target.
...
Caffold never restarts or updates the daemon on its own, including when the
installed and running versions differ. The daemon is user-global and may also
serve Codex clients outside Caffold, so the user must confirm an interruption
...
Settings → Codex reads `GET /api/codex/updates` when the page opens, on
Refresh, and after a restart or update finishes. Caffold does not check for a
newer Codex in the background or keep a copy of the answer. The report names
four facts, each from its own source:

- the installed Codex CLI version;
- the app-server version Caffold's latest readiness check saw running;
- the newest release on the `latest` channel at
  `https://releases.openai.com/codex/channels/latest`, which Codex's installer
  also resolves; and
- whether Codex's updater is on, read from the daemon settings file.

A source that cannot answer leaves its value out and puts its reason under
`problems`; the response is still `200`. The report's `update` is `available`
when the running app-server is not the installed version, or when the newest
release is newer than the running version, or than the installed version when
no running version is known. It is `upToDate` when both versions in that
comparison are known and neither holds, and `unknown` otherwise.

**Update Codex…** asks for its own confirmation, then calls
`POST /api/codex/update`. Caffold releases its proxy and marks subscribed
sessions unavailable as a restart does, with `Codex runtime is updating.`, and
runs `codex app-server daemon update` once. Codex installs the newest release
through its official installer and restarts a running daemon that is not on
that installation. The response keeps Codex's `status` (`updated`, `noUpdate`,
or `unsupported`), versions, and message. `noUpdate` means only that this run
installed nothing; Codex may still have restarted a daemon that was behind the
installation, and its message says whether it did. The command is bounded at
ten minutes, which covers the download and a shutdown grace period of up to
five minutes. The next status request reconnects; a failure leaves the proxy
released for that same recovery.

The control is available when canonical readiness is `ready` or
`restartRequired`, no restart or update is in flight, and `update` is
`available` or `unknown`; a report request that fails counts as `unknown`. The
browser admits one restart or update at a time, and the backend holds the same
runtime locks for both, so requests that need the Codex runtime wait until the
command finishes.
