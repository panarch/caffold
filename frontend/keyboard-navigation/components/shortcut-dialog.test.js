...
test("renders the complete current shortcut map in one native dialog", () => {
  const nativeDialog = {};
  const owner = {
  ...
  assert.match(owner.innerHTML, /Choosing a target/);
  assert.match(owner.innerHTML, /Scrolling/);
  for (const key of ["F", "S", "?", "J", "K", "D", "U", "H", "L"]) {
    assert.match(owner.innerHTML, new RegExp(`<kbd>${escapePattern(key)}</kbd>`));
  }
});
